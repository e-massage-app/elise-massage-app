...
// v1.0.9.2 : icone SVG (line) par groupe. stroke=currentColor -> herite la couleur du groupe.
function _annuGroupeIconSvg(nom) {
  const n = (nom || '').toLowerCase();
  const wrap = (inner) => `<svg class="annu-gi" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${inner}</svg>`;
  if (n.includes('headspa') || n.includes('head spa')) {
    return wrap('<path d="M12 2.5S5.5 9.5 5.5 14a6.5 6.5 0 0 0 13 0c0-4.5-6.5-11.5-6.5-11.5z"/>'); // goutte
  }
  if (n.includes('épil') || n.includes('epil')) {
    return wrap('<path d="M12 3l1.5 4L18 8.5 13.5 10 12 14l-1.5-4L6 8.5 10.5 7z"/><path d="M18.3 14l.7 1.9 1.9.7-1.9.7-.7 1.9-.7-1.9-1.9-.7 1.9-.7z"/>'); // etincelles
  }
  if (n.includes('massage') || n.includes('rituel') || n.includes('soin') || n.includes('spa')) {
    return wrap('<path d="M12 4c1.6 1.8 2.4 3.6 2.4 5.6 0 2-1 3.4-2.4 4.4-1.4-1-2.4-2.4-2.4-4.4C9.6 7.6 10.4 5.8 12 4z"/><path d="M6.5 8.5c2 .4 3.4 1.4 4.2 3 .8 1.6.8 3.4 0 5.4-2-.4-3.4-1.4-4.2-3-.8-1.6-.8-3.4 0-5.4z"/><path d="M17.5 8.5c-2 .4-3.4 1.4-4.2 3-.8 1.6-.8 3.4 0 5.4 2-.4 3.4-1.4 4.2-3 .8-1.6.8-3.4 0-5.4z"/>'); // lotus
  }
  return wrap('<circle cx="12" cy="12" r="4"/>'); // fallback
}

// v1.0.9.2 : icones SVG (line) pour les boutons d'action.
function _svgAction(kind) {
  const wrap = (inner) => `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${inner}</svg>`;
  if (kind === 'eye') return wrap('<path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/>');
  if (kind === 'edit') return wrap('<path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/>');
  if (kind === 'trash') return wrap('<path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/>');
  return '';
}

// Rend les pastilles "icone groupe + nombre" triees par volume decroissant.
  ...
  return `
    <tr class="${fideleClass}">
      ...
      <td class="actions-cell">
        <button class="annu-icon-btn" title="Consulter" onclick="ClientServices.showClientDetails('${client.id}')">${_svgAction('eye')}</button>
        <button class="annu-icon-btn" title="Éditer" onclick="FormManager.editClient('${client.id}')">${_svgAction('edit')}</button>
        <button class="annu-icon-btn danger" title="Supprimer" onclick="FormManager.deleteClient('${client.id}')">${_svgAction('trash')}</button>
      </td>
    </tr>
  `;
}
